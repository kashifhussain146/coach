...
if (! function_exists('module')) {
    ...
    {
      if($paginate !== ''){
        $array = ModulesData::with('results')->where('module_id',$id)->where('status','active')->paginate($paginate);
      }else{
        $array = ModulesData::with('results')->where('module_id',$id)->where('status','active')->get();
      }
