...
class moduleEditAuthorization
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        //     // Permission::firstOrCreate(['name' => "Module_Master_{$moduleSlug}"]);
        //     // $permission = Permission::firstOrCreate(['name' => "Module_Edit_{$moduleSlug}"]);

        //     $parent = Permission::firstOrCreate(['name' => "Module_Master_{$moduleSlug}",'parent_id'=>2]);
        //     $permission = Permission::firstOrCreate(['name' => "Module_Edit_{$moduleSlug}",'parent_id'=>$parent->id]);
        ...
        if (!auth()->guard('admin')->user()->can("Module_Edit_{$moduleSlug}")) {
