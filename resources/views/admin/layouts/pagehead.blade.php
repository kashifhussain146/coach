              ...
              @foreach (getNotifications() as $item)
                  ...
                  <li>
                    <a href="#" class="top-text-block">
                      <div class="top-text-heading">You recieved new  task <br /> Task ID : <b> {{$item->unique_group_id}} </b> </div>
                      <div class="top-text-light">Task Deadline : {{ date('d/m/Y H:i',strtotime($item->deadline_date_time))}} </div>
                    </a> 

                    <div class="top-text-block">
                      <a href="{{route('status-update',['id'=>$item->freelancers->first()->id,'status'=>\App\Models\Proposals::STATUS_ACCEPTED])}}"><button class="btn-sm btn btn-success ">I can do it</button></a> &nbsp;&nbsp;&nbsp;
                      <a href="{{route('status-update',['id'=>$item->freelancers->first()->id,'status'=>\App\Models\Proposals::STATUS_REJECTED])}}"><button class="btn-sm  btn btn-danger ">I can't do it</button>
                    </div>
                  </li>
                  <hr />
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu dropdown-menu-right user-dd animated flipInY">
            ...
            <div class="profile-dis scrollable">
                ...
                <button type="button" data-toggle="modal" data-target="#logoutModal" class="dropdown-item" > <i class="fa fa-power-off m-r-5 m-l-5"></i> Logout</button>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <a  class="btn btn-primary" href="#" onclick="event.preventDefault();document.getElementById('logout-form').submit();"> Yes </a>

          @if(Auth()->guard('admin')->user()->roles()->first()->name =='Free Lancer')
          <form id="logout-form" action="{{ route('freelancer.post.logout') }}" method="POST" style="display: none;">
          @else
