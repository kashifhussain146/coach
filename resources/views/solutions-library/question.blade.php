            ...
            <div class="my-3 text-center">
                ...
                @auth('web')
                    @if (!in_array($question->id, Auth::guard('web')->user()->isPaidQuestions()->pluck('question_id')->toArray()))
                        ...
                        <button data-question_id="{{ $question->id }}" data-question="{{ $question->question }}"
                            data-subject_category_id="{{ $question->subject_category }}"
                            data-subject_id="{{ $question->subject }}" data-price="{{ $question->price }}" type="button"
                            style="background-color: #ff7707; z-index: 10;"
                            class="BuyQuestionModal fs-6 px-4 py-1 text-white rounded-pill position-relative">Buy this Article
                            for ${{ $question->price }}</button>
                    @else
                ...
                @guest('web')
                    <button type="button" style="background-color: #ff7707; z-index: 10;" data-bs-toggle="modal"
                        data-bs-target="#LoginForm" class="fs-6 px-4 py-1 text-white rounded-pill position-relative">Buy this
                        Article for ${{ $question->price }}</button>
                @endguest
                        ...
                        <form method="POST" action="{{ route('cart.addToCart') }}">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body text-start">
                                    <p style="font-size: 1rem; line-height: 1.5;" class="mb-3 black fw-bold">The order has
                                        been purchased multiple times hence originality is not assured.</p>
                                    <p style="font-size: 0.9rem; line-height: 1.5;">If you wish to receive and original
                                        solution that is plagarism adn AI free click on order and original solution, Or else
                                        click Continue</p>
                                </div>
                                <div class="modal-footer">
                                    ...
                                    <button style="background-color: #ff7707;" type="submit"
                                        formaction="{{ route('checkout.question') }}" class="btn btn-primary">Order an
                                        original Solution</button>
                                </div>
                            </div>
