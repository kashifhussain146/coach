            ...
            <div class="px-4 cstm-txt-justify-mob">
                ...
                <div class="row w-75">
                    <div class="col-4 w-25">
                        <p style="background-color: #ff7707; font-size: small;"
                            class="text-white text-center rounded-pill">
                            {{ $v->category != '' ? $v->category->category_name : '' }}</p>
                    </div>
                    @if ($v->subjects != '')
                        <div class="col-4 w-25">
                            <p style="background-color: #ff7707; font-size: small;"
                                class="text-white text-center rounded-pill">
                                {{ $v->subjects != '' ? $v->subjects->subject_name : '' }}</p>
                        </div>
                    @endif
                    <div class="col-4 w-25">
                        <p style="background-color: #ff7707; font-size: small;"
                            class="text-white text-center rounded-pill"> Solutions ID {{ $v->id }}</p>
                    </div>
                </div>
                        ...
                        <div>
                            @auth('web')
                                <button type="button" style="background-color: #ff7707;float:right;margin-right:10px"
                                    class="BuyQuestionModal fs-6 px-3 fw-bold text-white"
                                    data-question_id="{{ $v->id }}" data-question="{{ $v->question }}"
                                    data-subject_category_id="{{ $v->subject_category }}"
                                    data-subject_id="{{ $v->subject }}"
                                    data-price="{{ $v->price }}">${{ $v->price }} | Buy Now
                                </button>
                            @endauth
